...
#ifdef RT_TSK_SET
  if(rt_task_policy == 1){
    struct timespec phaseDelay, periodDelay, nextWake;
	
	struct sched_attr attr;
	
	struct rt_args *task_args = thr;
	int period   = task_args->period;
	int deadline = task_args->deadline;
	int phase    = task_args->phase;
	void* task_func = task_args->task_func;
	
	phaseDelay.tv_sec   = (time_t)0;
	phaseDelay.tv_nsec  = phase * 1000000UL;	
	...
	periodDelay.tv_nsec = period * 1000000UL;
	 ...
	 attr.sched_runtime = task_args->wcet * 1000000UL;	
	 attr.sched_period = task_args->period * 1000000UL;	
	 attr.sched_deadline = task_args->deadline * 1000000UL;	
	 sched_setattr(0, &attr, 0);
	
	nextWake = tm0;
	TIMESPEC_ADD(nextWake, phaseDelay);
...
//__kmp_printf("\nlaunchworker1-3\n");
  ...
  //is this what allows the user code to exec?
  //NOPE, OCCASIONALLY ONE OF THE THREADS GOES OFF FIRST
...
void __kmp_create_worker(int gtid, kmp_info_t *th, size_t stack_size) {
  ...
  if (status != 0) {
	...
	if(rt_task_policy == 1){
		...
		struct sched_param param;
		task_args->period    = th->th_info->period;
		task_args->deadline  = th->th_info->deadline;
		task_args->phase     = th->th_info->phase;
		task_args->wcet      = th->th_info->wcet;
		task_args->task_func = &(th->th_info->task_func);
		pthread_attr_init(&thread_attr);
		pthread_attr_getschedparam(&thread_attr, &param);
		param.sched_priority = 99 - task_args->deadline;
		...
		status = pthread_create(&handle, &task_attr, __kmp_launch_worker, (void *)th);
	}else{
                status = pthread_create(&handle, &thread_attr, __kmp_launch_worker, (void *)th);
        }
        //*/
  if(rt_task_policy == 1){
    //code to make it rt (scheduler stuff)
    //pthread_attr_setschedpolicy(&thread_attr, SCHED_FIFO);
    //pthread_attr_setinheritsched(&thread_attr, PTHREAD_EXPLICIT_SCHED);
  }
  calls = calls + 1;
