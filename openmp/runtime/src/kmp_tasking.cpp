...
typedef struct rt_args{
  kmp_int32 gtid;
  kmp_task_t *task;
  //((void* (*)(void *)) task_routine;
  kmp_routine_entry_t task_routine;
};

//helper function to set up a shared wakeup time for all real-time tasks
void set_global_start(int seconds) {
  if (global_start_set == 1) {
    return;
  } else {
...
//this runs before *each* task
//unaffected by number of threads
static kmp_int32 __kmp_push_task(kmp_int32 gtid, kmp_task_t *task) {
  //__kmp_printf("\n+++++++++++pushtask: %d\n", gtid);
  //if(gtid==0){return(TASK_NOT_PUSHED);}
...
//invokes a task after it's been scheduled
...
#endif
  //task->rt_priority = 73; // FRANK SAYS: THIS OVERWRITES THE PRIORITY, NOT WCET!!!
  //task->wcet = 40; // FRANK SAYS: THIS OVERWRITES THE PRIORITY, NOT WCET!!!
...
//    TASK_CURRENT_QUEUED (1) if suspended and queued the current task to be
//    resumed later.
//here's where they try to queue/execute each task
...
kmp_int32 __kmpc_omp_task(ident_t *loc_ref, kmp_int32 gtid,
                          kmp_task_t *new_task) {
  //__kmp_printf("\nKMPCOMPTASK\n");
  new_task->rt_priority = (new_task->data2).priority;

  __kmp_printf("FIFO task priority3: %d\n", (new_task->data2).priority);

  new_task->is_edf = __kmp_check_edf(new_task->rt_priority);
  new_task->period = kmp_period(0);
  kmp_int32 res;
...
int __kmp_check_edf(kmp_int32 priority){
  if(priority > 40 && priority < 100){return(0);}else{return(1);}
}
